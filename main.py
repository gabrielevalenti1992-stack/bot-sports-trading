    ...
    try:
        if foto_path and os.path.exists(foto_path):
            ...
            with open(foto_path, 'rb') as photo:
                ...
                print(f"Telegram (con foto) - Status: {response.status_code}")
        else:
            url = f"https://api.telegram.org/bot{telegram_bot_token}/sendMessage"
            data = {'chat_id': telegram_chat_id, 'text': messaggio, 'parse_mode': 'Markdown'}
            response = requests.post(url, data=data, timeout=10)
            print(f"Telegram (solo testo) - Status: {response.status_code}")
...
def controlla_partita_fotmob(match_id):
    ...
    # 1. RECUPERO DATI DI BASE (SEPARATO E PROTETTO)
    try:
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code != 200:
            print(f"Errore HTTP FotMob: {response.status_code}")
            return None, None
        ...
        # Testo garantito: arriva sempre se la richiesta API va a buon fine
        messaggio = (
        ...
        print(f"Errore critico nel parsing dei dati base della partita: {e}")
        return None, None

    # 2. TENTATIVO GRAFICO MOMENTUM (SE FALLISCE, NON BLOCCA IL TESTO)
    foto_path = None
    try:
        ...
        if momentum_data:
            ...
            print("Momentum non disponibile per questo match: procedo solo con il testo.")
    except Exception as e:
        print(f"Generazione grafico momentum fallita, procedo comunque con il testo: {e}")
        foto_path = None
...
if __name__ == "__main__":
    while True:
        if fotmob_match_id:
            print("Controllo match su FotMob...")
            foto, testo = controlla_partita_fotmob(fotmob_match_id)
            if testo:
                invia_notifica_telegram(foto, testo)
            else:
                print("Nessun dato testuale disponibile in questo ciclo.")
        else:
